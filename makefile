...
ifndef version
	...
	@cd ./src && DOCKER_BUILDKIT=1 docker build --no-cache --build-arg VERSION=none -t dockware/shopware-essentials:$(version) .
...
ifndef tag
	...
	@cd ./src && DOCKER_BUILDKIT=1 docker build --no-cache --build-arg VERSION=$(CURRENT_SW_VERSION) --build-arg MIN_PHP=$(CURRENT_SW_VERSION_MIN_PHP) -t dockware/shopware:$(tag) .
